...
public:
	...
	int Recalc(int current_time, vector< pair <int, int>> &stop_density); //Переращет количества пассажиров на текущей остановке
	int landLoadPassangers(/*vector <pair<unsigned long, int>> &bus_intentions*/ vector<Passanger> &bus_pussangers, int &free_places, int current_time, vector <unsigned long> bus_stops, Parameters_ parameters, Bus &bus, int &max_passenger_time, int &max_waiting_time, vector< pair <int, int>> &avg_times, vector< pair <int, int>> &stop_density);	//Высадка и посадка пассажиров на остановке
	int RemainPassangers(vector< pair <int, int>> stop_density); //Возвращает количество пассажиров пришедших до конца дня после последнего автобуса
