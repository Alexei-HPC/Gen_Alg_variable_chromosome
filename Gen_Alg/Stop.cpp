...
/*
	...
	*/
	...
	if(free_places > 0)//Если есть свободные места в автобусе
	{
		...
		Recalc(curent_time, stop_density); //Пересчет количества пассажиров на остановке
		...
		while(free_places > 0 && passangers_to_this_bus > 0)
		{
			...
			// Подсчёт среднего времени ожидаия
...
int Stop::Recalc(int current_time, vector< pair <int, int>> &stop_density) //Перерасчет количества пассажиров на текущей остановке
{
...
int Stop::RemainPassangers(vector< pair <int, int>> stop_density)	//Возвращает количество пассажиров пришедших до конца дня после последнего автобуса
{
